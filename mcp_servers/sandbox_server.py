...
def make_sandbox_server(repo_root: Path) -> FastMCP:
    ...
    @mcp.tool(
        name="run_target_tests", description="Run the fixed public pytest profile in the sandbox."
    )
    ...
    @mcp.tool(
        name="run_regression", description="Run the fixed regression pytest profile in the sandbox."
    )
